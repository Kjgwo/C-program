...
void TypingSystem::printMenu()
{
	...
	putStringOnPosition(6, 12, "2. 메뉴를 입력해 주세요(종료는X 입력):");
}
...
void TypingSystem::typingStart()
{
	...
	for (int i = 0; i < 5; i++) {
		...
		currentUser->avgSpeed = (currentUser->avgSpeed * ((float)currentUser->typingCount - 1.0) + types_per_min) / currentUser->typingCount;
